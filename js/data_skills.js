...
const skillDefs = {
  ...
  whirlwind:      { name:'Whirlwind',      desc:'A flurry of 4 melee strikes (~0.35× damage each).',                           req:{STR:13}, cost:4, icon:'🌀', classRestriction:['warrior','paladin'], cooldown:4,  manaCost:15, effect:{type:'damage', aoe:true, mult:0.5} },
  ...
  pick_lock:      { name:'Pick Lock',      desc:'Instantly open locked doors and chests.',           req:{DEX:11}, cost:1, icon:'🔓', classRestriction:['rogue','ranger'],   cooldown:0,  manaCost:5,  noCombat:true, effect:{type:'special', openLocked:true} },
  ...
  peek:           { name:'Peek',           desc:'Reveal hidden traps and secrets in the area.',      req:{WIS:10}, cost:1, icon:'👁️', classRestriction:['rogue','ranger'],   cooldown:0,  manaCost:0,  noCombat:true, effect:{type:'special', revealHidden:true} },
  track:          { name:'Track',          desc:'Highlight all enemies on the current map.',         req:{WIS:10}, cost:1, icon:'🐾', classRestriction:['rogue','ranger'],   cooldown:0,  manaCost:3,  noCombat:true, effect:{type:'special', revealEnemies:true} },
  ...
  dispel_magic:   { name:'Dispel Magic',   desc:"Weaken the enemy's attacks by 10% for 4 turns.",          req:{INT:13}, cost:3, icon:'✨', classRestriction:['mage'],            cooldown:4,  manaCost:15, effect:{type:'debuff', removeEnemyBuffs:true} },
  ...
  meteor_swarm:   { name:'Meteor Swarm',   desc:'Rain 2 meteors for heavy INT-based magic damage.',                    req:{INT:16}, cost:6, icon:'☄️', classRestriction:['mage'],            cooldown:8,  manaCost:50, effect:{type:'damage', aoe:true, mult:3.0, statScaling:'INT'} },
  ...
  // ── RANGER / NATURE ────────────────────────────────────────────────────────
  call_lightning: { name:'Call Lightning', desc:'Strike with 3 bolts of lightning, scaling with DEX.',               req:{DEX:13}, cost:3, icon:'⛈️', classRestriction:['ranger'],           cooldown:4,  manaCost:18, effect:{type:'damage', aoe:true, mult:2.0, statScaling:'DEX'} },
  ...
  forage:         { name:'Forage',         desc:'Find a random herb or consumable item.',          req:{WIS:10}, cost:1, icon:'🌾', classRestriction:['ranger'],           cooldown:0,  manaCost:0,  noCombat:true, effect:{type:'special', findItem:'herb'} },
  ...
  slow:           { name:'Slow',           desc:'Slow the enemy, reducing its attack by 20% for 2 turns.',req:{INT:12}, cost:2, icon:'🐌', classRestriction:['mage','rogue'],     cooldown:4,  manaCost:15, effect:{type:'debuff', slowEnemy:true, duration:2} },
  fly:            { name:'Fly',            desc:'Ignore terrain effects for 3 turns.',            req:{DEX:11}, cost:2, icon:'🦅', classRestriction:['mage','rogue'],     cooldown:5,  manaCost:8,  noCombat:true, effect:{type:'buff', ignoreTerrain:true, duration:3} },
